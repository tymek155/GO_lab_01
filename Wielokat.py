...
class Wielokat:
    ...
    def sprawdz_wielokat(self):
        ...
        for i in range(0, len(lista_linii)):
            if i == (len(lista_linii) - 1):
                ...
                if punkt_przeciecia is not None and punkt_przeciecia.__eq__n__(lista_linii[i].pkt_2)  and punkt_przeciecia.__eq__n__(lista_linii[0].pkt_1):
            ...
            if punkt_przeciecia is None or (punkt_przeciecia.__eq__n__(lista_linii[i].pkt_2) == 0 and punkt_przeciecia.__eq__n__(lista_linii[i+1].pkt_1) == 0):
                print("Nie można utowrzyć wielokąta!")
                return None
    ...
    def sprawdz_przynaleznosc_punktu(self, punkt):
        ...
        for i in range(0, len(lista_linii)):
            ...
            if opt == True:
                if (punkt_przeciecia.__eq__n__(lista_linii[i].pkt_1) == 0) and (punkt_przeciecia.__eq__n__(lista_linii[i].pkt_2) == 0):
                    przeciecia += 1
                else:
                    if (i+1)>=len(lista_linii):
                        if ((lista_linii[i].pkt_1.y <punkt_przeciecia.y) and (lista_linii[0].pkt_2.y >punkt_przeciecia.y) or (lista_linii[i].pkt_1.y > punkt_przeciecia.y) and (lista_linii[0].pkt_2.y < punkt_przeciecia.y)):
                            przeciecia += 1
                            i = i + 1
                        else:
                            i = i + 1
                    else:
                        if ((lista_linii[i].pkt_1.y <punkt_przeciecia.y) and (lista_linii[i+1].pkt_2.y >punkt_przeciecia.y) or (lista_linii[i].pkt_1.y > punkt_przeciecia.y) and (lista_linii[i+1].pkt_2.y < punkt_przeciecia.y)):
                            przeciecia += 1
                            i = i +2
                        else:
                            i = i + 2
